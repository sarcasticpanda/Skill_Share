...
try {
    ...
    $totalUsers = mysqli_fetch_array($result)[0];
} catch (Exception $e) {
...
try {
    ...
    $totalSkills = mysqli_fetch_array($result)[0];
} catch (Exception $e) {
...
try {
    $result = mysqli_query($conn, "SELECT COUNT(DISTINCT LEAST(from_id, to_id), GREATEST(from_id, to_id)) FROM messages");
    $totalConnections = mysqli_fetch_array($result)[0];
} catch (Exception $e) {
